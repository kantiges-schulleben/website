...
function startup() {
    // Reden/Statements ===================================================================================
    $.get('/blog/internationales/getArticles/0', (json: { [key: string]: any }[]) => {
        let count: number = 0;
        if (json.length > 0) {
                ...
                (
                    ...
                    createStatement(
                        ...
                        decodeURIComponent(article.image) != ''
                            ? decodeURIComponent(article.image)
                            : '/images/default.png',
                        `/blog/internationales/view/${decodeURIComponent(article.id)}`
                    )
                );
...
function makeCallAndUpdateDOM() {
    $.get(
        `/blog/internationales/getArticles/${page}`,
        (json: { [key: string]: any }[]) => {
            let count: number = 0;
            if (json.length > 0) {
                json.forEach((article: { [key: string]: any }) => {
